...
const addLanguagelevel = async (req, res) => {
  const { language_levelid, language_level, language_id } = req.body;

  if (!language_id) {
    return res
      .status(400)
      .json({ message: "Vui lòng chọn ngôn ngữ cho trình độ này" });
  }

  if (language_levelid && /[^A-Z0-9]/.test(language_levelid)) {
    return res.status(400).json({
      message: "Mã trình độ phải là chữ in hoa và không chứa ký tự đặc biệt",
    });
  }
...
const deleteMultipleLanguagelevel = async (req, res) => {
  try {
    const result = await languageLevelService.deleteMany(
      req.body.languagelevelIds
    );

    if (!result.success) {
      return res.status(400).json({
        message:
          "Cannot delete. This languagelevel is currently in use for another course.",
